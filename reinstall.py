...
def windows_add_to_path(executable_path: Path):
    ...
    with winreg.OpenKey(
        ...
        if new_path not in user_path:
            ...
            print("swami already in path but not present, installing..")
...
def main():
    ...
    if Path(INSTALLDIR).exists():
        print("removing installation as it is already present and installing current one")
        import shutil
        import stat
        ...
        def onerror(func, path, exc_info):
            ...
            func(path)
        shutil.rmtree(INSTALLDIR, onerror=onerror)
