...
int sizeof_string = (3 * sizeof(int));
...
func ptr char read_file(ptr char filename, ptr int size) {
    ...
    ptr char buffer = cast(ptr char, malloc(size[0]+1));
    if !buffer { fclose(file) return nullptr; }

    fread(buffer, 1, size[0], file)
    buffer[size[0]] = cast(char, 0);
...
func void str_append(ptr string __str, char c) {
    ...
    _str.length++;
    _str.data[_str.length] = cast(char, 0);
    __str[0] = _str;
    return;
}
...
func void str_add(ptr string __str, ptr char cstr) {
    ...
    memcpy(cast(ptr char, _str.length+cast(int, _str.data)), cstr, clen);
